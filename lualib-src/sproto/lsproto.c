...
lnewproto(lua_State *L) {
	struct sproto * sp;
	size_t sz;
	void * buffer = (void *)luaL_checklstring(L,1,&sz);		// 这个字符串是已经通过 sprotoparser.parse 转化过的二进制字符串
...
luaopen_sproto_core(lua_State *L) {
	...
	luaL_Reg l[] = {
		...
		// 通过模式字符串创建 1 个 sproto 对象, 该模式字符串是通过 sprotoparser.parse 生成的字符串.
		{ "newproto", lnewproto },
		...
		// 从 sproto 对象中根据类型名字查询 1 个 struct sproto_type 对象
